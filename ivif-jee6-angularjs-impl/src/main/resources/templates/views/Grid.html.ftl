...
<#if editable || grid.element.toolbar?exists>
    ...
    <#list grid.toolbarButtons as toolbarButton>
                ...
                <table ng-table="${grid.variableName}TableParams" show-filter="true" class="table table-bordered table-condensed table-hover table-striped">
                    <tr ng-repeat="line in $data"<#if grid.element.onOpen?has_content || grid.singleSelection> ng-click="clickLine(line)"</#if><#if grid.singleSelection> ng-class="{'active': line === selectedLine}"</#if>>
